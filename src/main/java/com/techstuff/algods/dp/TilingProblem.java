...
public class TilingProblem {
	...
	public int countWays() {
		...
		countMap.put(1, 1);
		countMap.put(2, 2);
		return countWays(n, countMap);
	}
	
	private int countWays(int num, Map<Integer, Integer> countMap) {
		if(countMap.containsKey(num)) {
			return countMap.get(num);
		}
		int count = countWays(num - 1, countMap) + countWays(num - 2, countMap);
